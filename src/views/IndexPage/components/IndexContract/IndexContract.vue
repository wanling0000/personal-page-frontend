...
async function send() {
	if (name.value === "") {
		messageDealer.showMsg("Name cannot be empty (;△;)", "error");
		return;
	}
	if (email.value === "") {
		messageDealer.showMsg("Email cannot be empty (;△;)", "error");
		return;
	} else {
		let regex = /^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/;
		let isValidEmail = regex.test(email.value);
		if (!isValidEmail) {
			messageDealer.showMsg("Please check if the email is correct (;△;)", "error");
			return;
		}
	}
	if (message.value === "") {
		messageDealer.showMsg("Message cannot be empty (;△;)", "error");
		return;
	}
	...
	await sendEmail({
		...
		.then(() => {
			messageDealer.showMsg("Sent successfully ٩( ╹▿╹ )۶", "success");
		})
		.catch((error) => {
			messageDealer.showMsg("Failed to send (´；ェ；`)", "error");
			console.log(error);
		});
	...
	<div class="index-contract-container" ref="baseContainer">
		...
		<div class="contract-container" ref="contractContainer">
			...
			<div class="contract-item">
				<span class="contract-item-name">Your Name: </span>
				<input
					type="text"
			...
			<div class="contract-item">
				<span class="contract-item-name">Your Email: </span>
				<input
					type="text"
					class="contract-item-input"
					placeholder="E-mail"
			...
			<div class="contract-item">
				<span class="contract-item-name">Anything you'd like to say: </span>
				<input
					type="text"
					class="contract-item-input"
